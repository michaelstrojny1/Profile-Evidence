#!/usr/bin/env python3
"""
Profile Evidence vs Profile Likelihood (Oscillator)
...
class CFG:
    # Model parameters
    omega_true: float = 1.9
    gamma_true: float = 0.1
    zeta_true: float = 0.5
    
    # Simulation
    dt: float = 0.05
    T_analysis: int = 200  # Time series length for analysis
    x0 = np.array([1.0, 0.0], dtype=DTYPE)
    q_std: float = 0.1
    r_std: float = 0.05
    r_std2: float = 0.45  # higher noise on second channel to reduce identifiability
    
    # Priors (weakly informative)
    prior_mean = np.array([0.1, 0.5], dtype=DTYPE)
    prior_cov = np.array([[0.01, 0.0], [0.0, 0.25]], dtype=DTYPE)
    bounds = [(0.01, 0.5), (0.1, 2.0)]  # [gamma, zeta]
    
    # Compensation demo (makes nuisance amplitude vary in-time to create compensation opportunity)
    enable_comp_demo: bool = True
    # Nuisance amplitude noise distribution for zeta dynamics ('gaussian' | 'laplace' | 'student_t')
    zeta_noise_dist: str = 'gaussian'
    zeta_t_nu: float = 4.0  # degrees of freedom for Student-t (nu>2)
    zeta_jitter_std: float = 0.30  # std of per-step zeta perturbation (model assumes constant zeta)
    zeta_ar1_rho: float = 0.85
    zeta_walk_std: float = 0.35
    enable_q_corr: bool = True
    q_corr: float = 0.7  # correlation between process noise dims in simulation only

def A_matrix(omega: float, gamma: float, dt: float) -> np.ndarray:
    """State transition matrix for damped oscillator."""
    return np.array([
        [1.0, dt],
        [-omega**2 * dt, 1.0 - gamma * dt]
    ], dtype=DTYPE)
...
def simulate(omega: float, gamma: float, zeta: float, T: int, dt: float,
             x0: np.ndarray, q_std: float, r_std: float, seed: int = 0) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    """Simulate 2D damped oscillator with correlated forcing."""
    rng = np.random.default_rng(seed)
    A = A_matrix(omega, gamma, dt)
    ...
    zeta_eff = zeta
    zeta_trace = np.zeros(T, dtype=DTYPE)
    for t in range(T):
        # Correlated forcing with optional compensation dynamics in amplitude
        if CFG.enable_comp_demo:
            # AR(1) drift in zeta to create persistent compensation opportunities
        ...
        forcing = zeta_eff * omega * np.sin(omega * (t * dt))
        
        # Dynamics
        eps = rng.normal(0, 1.0, 2).astype(DTYPE)
        proc_noise = (Qchol @ eps) * q_std
        x = A @ x + np.array([0.0, forcing * dt], dtype=DTYPE) + proc_noise
...
def kalman_loglik(y: np.ndarray, omega: float, gamma: float, zeta: float, dt: float,
    ...
    for t in range(T):
        forcing = zeta * omega * np.sin(omega * (t * dt))
        
        # Predict
        x_pred = A @ x + np.array([0.0, forcing * dt], dtype=DTYPE)
        P_pred = A @ P @ A.T + Q
...
def log_prior_psi(psi: np.ndarray) -> float:
    """Log prior density for nuisance parameters.
    
    Args:
        psi: Nuisance parameter vector [gamma, zeta]
...
def log_gaussian(x: np.ndarray, mean: np.ndarray, cov: np.ndarray) -> float:
    """Log Gaussian density with robust numerical computation.
...
def run_bulletproof(seed: int = 0, make_plots: bool = True, write_files: bool = True):
    print("=" * 80)
    print("PROFILE EVIDENCE vs PROFILE LIKELIHOOD (Oscillator)")
    print("=" * 80)
    ...
    try:
        xs, ys, zeta_trace = simulate(CFG.omega_true, CFG.gamma_true, CFG.zeta_true, T, dt, CFG.x0, CFG.q_std, CFG.r_std, seed)
    ...
    # Omega domain bounds
    omega_min, omega_max = 1.4, 2.2
    
    # Coarse grid to find peak (denser)
    print("Starting coarse grid search...")
    coarse = np.linspace(omega_min, omega_max, 33, dtype=DTYPE)
    pl_coarse = []
    ...
    grid_half_width = max(0.15, abs(w_star - CFG.omega_true) + 0.05)
    refine_unclamped = np.linspace(w_star - grid_half_width, w_star + grid_half_width, 21, dtype=DTYPE)
    # Clamp refined grid to domain bounds
    refine = np.clip(refine_unclamped, omega_min, omega_max)
    
    baseN = 2000  # Fixed N (faster demo; compensation is strong)
    # Lean scale grid (faster) while allowing visible differences
    proposal_scales = [1.0, 2.0, 4.0, 8.0]
    ...
    for i, w in enumerate(refine):
        ...
        if ess < 0.1:
            ...
            pe_refined, _, ess_refined, best_scale_refined, samples_refined, weights_refined = compute_pe_given_cov(
                ys, float(w), psi_map, Sigma, dt, q_var, r_var, baseN, refined_scales, rng_seed=42+i+100
            )
            if ess_refined > ess:
                pe, ess, best_scale, samples, weights = pe_refined, ess_refined, best_scale_refined, samples_refined, weights_refined
    ...
    ax2.set_xlabel('omega'); ax2.set_ylabel('ESS'); ax2.legend(); ax2.grid(True, alpha=0.3)

    ax3 = axes[1, 0]
    ax3.plot(refine, best_scales, 'purple', marker='o')
    ax3.set_title('Optimal Proposal Scale Factor per omega')
    ax3.set_xlabel('omega'); ax3.set_ylabel('Scale Factor'); ax3.grid(True, alpha=0.3)
    ...
    ax4.set_xlabel('omega'); ax4.set_ylabel('PL'); ax4_t.set_ylabel('PE')
    ...
    if make_plots:
        ...
        axc1.set_xlabel('omega'); axc1.set_ylabel('Gain')
        axc1.grid(True, alpha=0.3)

        axc2 = axes2[0, 1]
        axc2.plot(refine, ridge_logvols, 'c-s')
        axc2.set_title('Ridge Log-Volume (0.5[d log 2*pi + logdetSigma])')
        axc2.set_xlabel('omega'); axc2.set_ylabel('Log-Volume')
        axc2.grid(True, alpha=0.3)
        try:
            axc2_t = axc2.twinx()
            axc2_t.plot(refine, np.log10(np.asarray(hess_conds)), 'r--', linewidth=1.0)
            axc2_t.set_ylabel('log10 cond(H)')
        except Exception:
            pass
        ...
        axc3.set_xlabel('omega'); axc3.set_ylabel('Mahalanobis^2')
        ...
        axd.set_xlabel('omega'); axd.set_ylabel('Delta (PE - PL)')
        ...
        for w_idx, w_val in enumerate(refine):
            if w_val in high_pos_delta_omegas:
                ...
                axc.set_title(f'IS at omega={w_val:.3f} (delta={delta_val:+.3f}; ESS={ess_val:.3f} {status}; N={baseN}; scale={scale_val:.1f}{title_theta}{title_extra})')
                axc.legend()
        ...
        for w_idx, w_val in enumerate(refine):
            if w_val in high_neg_delta_omegas:
                ...
                axc2.set_title(f'IS at omega={w_val:.3f} (delta={delta_val:+.3f}; ESS={ess_val:.3f} {status}; N={baseN}; scale={scale_val:.1f}{title_theta2}{title_extra2})')
                axc2.legend()
                axc2.grid(True, alpha=0.3)
        ...
        try:
            def compute_innovations(y_in: np.ndarray, w_in: float, psi_in: np.ndarray) -> np.ndarray:
                Tloc = y_in.shape[0]
                A = A_matrix(w_in, float(psi_in[0]), dt)
                Q = q_var * np.eye(2, dtype=DTYPE)
                R = np.array([[r_var, 0.0],[0.0, (CFG.r_std2**2)]], dtype=DTYPE)
                x = np.zeros(2, dtype=DTYPE)
                P = 0.2 * np.eye(2, dtype=DTYPE)
                innovs = []
                for t in range(Tloc):
                    forcing = float(psi_in[1]) * w_in * np.sin(w_in * (t * dt))
                    x_pred = A @ x + np.array([0.0, forcing * dt], dtype=DTYPE)
                    P_pred = A @ P @ A.T + Q
                    S = P_pred + R
                    try:
            ...
            if n > 0:
                ...
                axs[0].set_title('RMSE of innovations vs omega')
                axs[0].set_xlabel('omega'); axs[0].set_ylabel('RMSE')
    ...
    if write_files:
        ...
        # Save concise overestimation summary (peak negative delta)
        try:
    ...
    # Area metrics for delta_norm
    try:
        pos_area = float(np.trapz(np.clip(delta_norm, 0.0, None), refine))
        neg_area = float(np.trapz(np.clip(-delta_norm, 0.0, None), refine))
    except Exception:
        pos_area, neg_area = np.nan, np.nan
    ...
    results = {
        ...
        'delta_pos_area': pos_area,
        'delta_neg_area': neg_area,
        ...
        'rmse_curve': rmse_curve if 'rmse_curve' in locals() else None,
    ...
    if write_files:
        ...
        try:
            manifest = {
                'model': os.environ.get('MODEL_NAME', 'oscillator'),
                'seed': int(seed),
                'config': {
        ...
        # Write zeta_trace statistics for transparency
        try:
            zeta_mean = float(np.mean(zeta_trace))
            zeta_std = float(np.std(zeta_trace))
            zeta_min = float(np.min(zeta_trace))
            zeta_max = float(np.max(zeta_trace))
            stats_path = f"{get_run_prefix()}zeta_trace_stats.txt"
            with open(stats_path, 'w', encoding='utf-8') as fh:
                fh.write(f"zeta_trace mean={zeta_mean:.6f}, std={zeta_std:.6f}, min={zeta_min:.6f}, max={zeta_max:.6f}\n")
            print(f"Saved zeta_trace stats: {stats_path}")
        except Exception as e:
            print(f"zeta_trace stats write failed: {e}")

    # Return results for programmatic use
    results['delta_norm'] = delta_norm
    return results

def run_profile_evidence(seed: int = 0, make_plots: bool = True, write_files: bool = True):
    """Compatibility wrapper for runners expecting run_profile_evidence()."""
    return run_bulletproof(seed=seed, make_plots=make_plots, write_files=write_files)
