#!/usr/bin/env python3
"""
Profile Evidence vs Profile Likelihood (Single-Area LFC)
...
class CFG:
    # LFC small-signal parameters (2-state model)
    # Parameter of interest: droop R (pu/Hz)
    omega_true: float = 2.0  # interpret as R (droop). Larger R -> weaker primary control
    # Nuisance parameters: D (load damping) and Tg (governor time constant)
    gamma_true: float = 0.8  # D (load damping)
    zeta_true: float = 0.5   # Tg (governor time constant, seconds)
    M_inertia: float = 8.0   # inertia constant (aggregated), seconds
    ...
    bounds = [(0.05, 3.0), (0.1, 2.0)]  # D in [0.05,3.0], Tg in [0.1,2.0]
    ...
    # Load disturbance (unmodeled in inference) to induce mild mismatch
    dPd_enable: bool = True
    dPd_ar1_rho: float = 0.95
    dPd_walk_std: float = 0.02
    dPd_jitter_std: float = 0.01

def A_matrix(droop_R: float, damping_D: float, dt: float, Tg: float, M: float) -> np.ndarray:
    """Discrete-time state transition for simplified single-area LFC (2-state).
    States: x = [Δf, ΔPg]^T
      Δf_dot = (1/M) * (ΔPg - D Δf - ΔPd)
      ΔPg_dot = (1/Tg) * ( -ΔPg - (1/R) Δf + u )
    Forward Euler discretization.
    """
    a11 = 1.0 + dt * (-(damping_D / M))
    a12 = dt * (1.0 / M)
    a21 = dt * (-(1.0 / (max(droop_R, 1e-6) * max(Tg, 1e-6))))
    a22 = 1.0 - dt * (1.0 / max(Tg, 1e-6))
    return np.array([[a11, a12], [a21, a22]], dtype=DTYPE)

def B_vector(Tg: float, dt: float) -> np.ndarray:
    """Input vector for control setpoint u entering governor channel."""
    return np.array([0.0, dt * (1.0 / max(Tg, 1e-6))], dtype=DTYPE)

def generate_input(T: int, dt: float) -> np.ndarray:
    """Generate known excitation u_t used in both simulation and inference."""
    u = np.zeros(T, dtype=DTYPE)
    if CFG.input_type == 'step':
        t0 = int(max(0, CFG.input_t0))
        if t0 < T:
            u[t0:] = CFG.input_amp
    elif CFG.input_type == 'chirp':
        t = np.arange(T, dtype=DTYPE) * dt
        f0, f1 = 0.1, 1.0
        u = CFG.input_amp * np.sin(2.0 * np.pi * (f0 + (f1 - f0) * (t / max(t[-1], 1e-6))) * t)
...
def simulate(omega: float, gamma: float, zeta: float, T: int, dt: float,
    ...
    """Simulate simplified single-area LFC with optional time-varying Tg and load disturbance."""
    ...
    zeta_eff = zeta  # effective Tg
    dPd_eff = 0.0
    u = generate_input(T, dt)
    for t in range(T):
        # Time-varying governor constant Tg and unmodeled load disturbance ΔPd to create compensation
        if CFG.enable_comp_demo:
            ...
            if CFG.dPd_enable:
                dPd_eff = CFG.dPd_ar1_rho * dPd_eff \
                           + _draw_noise(rng, CFG.dPd_walk_std, 'gaussian', 4.0) \
                           + _draw_noise(rng, CFG.dPd_jitter_std, 'gaussian', 4.0)
        # Clamp effective Tg to physical bounds to avoid instability
        zeta_eff = float(np.clip(zeta_eff, CFG.bounds[1][0], CFG.bounds[1][1]))
        # Clamp disturbance magnitude to avoid numeric blow-ups
        if CFG.dPd_enable:
            dPd_eff = float(np.clip(dPd_eff, -0.5, 0.5))
        # Dynamics with known excitation u[t]
        A_t = A_matrix(omega, gamma, dt, zeta_eff, CFG.M_inertia)
        B_t = B_vector(zeta_eff, dt)
        eps = rng.normal(0, 1.0, 2).astype(DTYPE)
        proc_noise = (Qchol @ eps) * q_std
        x = A_t @ x + B_t * u[t] + proc_noise
        # Add unmodeled load disturbance into frequency channel
        if CFG.enable_comp_demo and CFG.dPd_enable:
            x[0] += float((-dt / max(CFG.M_inertia, 1e-6)) * dPd_eff)
...
def kalman_loglik(y: np.ndarray, omega: float, gamma: float, zeta: float, dt: float,
    ...
    """Kalman filter predictive log-likelihood for LFC linear model with known input."""
    T = y.shape[0]
...
def run_bulletproof(seed: int = 0, make_plots: bool = True, write_files: bool = True):
    print("=" * 80)
    print("PROFILE EVIDENCE vs PROFILE LIKELIHOOD (Single-Area LFC)")
    print("=" * 80)
    ...
    print("Starting coarse grid search (R droop)...")
    ...
    for i, w in enumerate(coarse):
        print(f"Coarse [{i+1:02d}/{len(coarse)}] R={w:.3f}", end=" ", flush=True)
    ...
    print(f"PL peak found at R = {w_star:.3f}")
    ...
    try:
        print(f"Refine grid range: [{float(refine[0]):.3f}, {float(refine[-1]):.3f}], true R={CFG.omega_true:.3f}, points={len(refine)}")
    except Exception:
        pass
    ...
    for i, w in enumerate(refine):
        print(f"  [{i+1:02d}/{len(refine)}] R={float(w):.3f}")
    ...
    ax1.axvline(CFG.omega_true, color='green', linestyle='--', label='True R', alpha=0.8)
    ...
    ax2.set_title('Relative ESS per R (Raw IS)')
    ax2.set_xlabel('R (droop)'); ax2.set_ylabel('ESS'); ax2.legend(); ax2.grid(True, alpha=0.3)

    ax3 = axes[1, 0]
    ax3.plot(refine, best_scales, 'purple', marker='o')
    ax3.set_title('Optimal Proposal Scale Factor per R')
    ax3.set_xlabel('R (droop)'); ax3.set_ylabel('Scale Factor'); ax3.grid(True, alpha=0.3)
    ...
    try:
        ax4.axvline(CFG.omega_true, color='green', linestyle='--', alpha=0.6)
    except Exception:
        pass
    ax4.set_title('Raw Values (different scales)')
    ax4.set_xlabel('R (droop)'); ax4.set_ylabel('PL'); ax4_t.set_ylabel('PE')
    ...
    if make_plots:
        ...
        try:
            axd.axvline(CFG.omega_true, color='green', linestyle='--', alpha=0.6)
        except Exception:
            pass
        axd.set_title('Correction Delta (PE_norm - PL_norm)')
        axd.set_xlabel('R (droop)'); axd.set_ylabel('Delta (PE - PL)')
        ...
        # Compensation cloud plots at high positive-delta points (PE > PL)
        high_pos_delta_omegas = refine[np.where(delta_norm > 0.05)[0]]
        if high_pos_delta_omegas.size == 0 and refine.size > 0:
            # Fallback: use top-3 delta points so clouds exist
            top_k = min(3, refine.size)
            top_idx = np.argsort(-delta_norm)[:top_k]
            high_pos_delta_omegas = refine[top_idx]
        for w_idx, w_val in enumerate(refine):
            if w_val in high_pos_delta_omegas:
                ...
                axc.set_title(f'IS at R={w_val:.3f} (delta={delta_val:+.3f}; ESS={ess_val:.3f} {status}; N={baseN}; scale={scale_val:.1f}{title_theta}{title_extra})')
                axc.legend()
        ...
        # Overestimation cloud plots at high negative-delta points (PE < PL)
        high_neg_delta_omegas = refine[np.where(delta_norm < -0.05)[0]]
        if high_neg_delta_omegas.size == 0 and refine.size > 0:
            # Fallback: use bottom-3 delta points
            top_k_neg = min(3, refine.size)
            bot_idx = np.argsort(delta_norm)[:top_k_neg]
            high_neg_delta_omegas = refine[bot_idx]
        for w_idx, w_val in enumerate(refine):
            if w_val in high_neg_delta_omegas:
                ...
                axc2.set_title(f'IS at R={w_val:.3f} (delta={delta_val:+.3f}; ESS={ess_val:.3f} {status}; N={baseN}; scale={scale_val:.1f}{title_theta2}{title_extra2})')
                axc2.legend()
                axc2.grid(True, alpha=0.3)
        ...
        try:
            ...
            if n > 0:
                ...
                axs[0].set_title('RMSE of innovations vs R')
                axs[0].set_xlabel('R (droop)'); axs[0].set_ylabel('RMSE')
    ...
    if write_files:
        ...
        try:
            ...
            manifest = {
                'model': os.environ.get('MODEL_NAME', 'lfc'),
                'seed': int(seed),
                'config': {
